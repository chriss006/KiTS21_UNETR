...
def get_xy_image_list(dir):
    ...
    train_input_path = dir + '/images/*.*'
    train_label_path = dir + '/labels/*.*'
    ...
    print('Input images loaded: {} -- Label images loaded: {}\n\tpath: {}'.format(
        len(train_input_filenames), len(train_label_filenames), dir))
    ...
    # Return both images and file paths
    return train_img, train_lbl, train_input_filenames, train_label_filenames
...
def get_train_val_generators(X_data, Y_data, validation_split=0.25,
                             batch_size=16, seed=42, rotation_range=0,
    ...
    if val_preprocessing_function is not None:
    ...
    def train_generator():
        while True:
            x_batch = X_train_augmented.__next__()
            y_batch = Y_train_augmented.__next__()
            yield x_batch, y_batch

    def val_generator():
        while True:
            x_batch = X_test_augmented.__next__()
            y_batch = Y_test_augmented.__next__()
            yield x_batch, y_batch
    train_gen = train_generator()
    val_gen = val_generator()

    # Train generator에서 첫 배치 가져오기
    try:
        x_batch, y_batch = next(train_gen)
        print(f"Train batch shapes: x={x_batch.shape}, y={y_batch.shape}")
    except Exception as e:
        print(f"Error in train_generator: {e}")

    # Validation generator에서 첫 배치 가져오기
    try:
        x_val_batch, y_val_batch = next(val_gen)
        print(f"Validation batch shapes: x={x_val_batch.shape}, y={y_val_batch.shape}")
    except Exception as e:
        print(f"Error in val_generator: {e}")
